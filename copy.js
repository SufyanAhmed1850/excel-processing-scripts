...
const copyDataToNewWorkbook = async () => {
    XlsxPopulate.fromBlankAsync()
        .then((workbook) => {
            const temperatureSheet = workbook.sheet(0).name("Temperature");
            const humiditySheet = workbook.addSheet("Humidity");

            sensorData.forEach((logger, ind) => {
                let columnLetter = getColumnLetter(ind);
                logger.temperatures.forEach((val, i) => {
                    if (i === 0) {
                        const cell = temperatureSheet.cell(
                            columnLetter + (i + 1)
                        );
                    ...
                    temperatureSheet.cell(columnLetter + (i + 2)).value(val);
                });
                logger.humidities.forEach((val, i) => {
                    if (i === 0) {
                        const cell = humiditySheet.cell(columnLetter + (i + 1));
                        cell.value(logger.fileName.split(".")[0]);
                        cell.style({
                            fill: "000000", // Black background
                            fontColor: "FFFFFF", // White text
                    ...
                    humiditySheet.cell(columnLetter + (i + 2)).value(val);
                });
                temperatureSheet
                    .cell(columnLetter + 676)
                    .formula(
                        "MIN(" + columnLetter + "2:" + columnLetter + "674)"
                    )
                    .style("numberFormat", "0.0");
                temperatureSheet
                    .cell(columnLetter + 677)
                    .formula(
                        "MAX(" + columnLetter + "2:" + columnLetter + "674)"
                    )
                    .style("numberFormat", "0.0");
                temperatureSheet
                    .cell(columnLetter + 678)
                    .formula(
                        "AVERAGE(" + columnLetter + "2:" + columnLetter + "674)"
                    )
                    .style("numberFormat", "0.0");
                humiditySheet
                    .cell(columnLetter + 676)
            ...
            workbook
                .toFileAsync("./Final/Data.xlsx")
                .then(() =>
                    console.log(
                        "Sensor data file saved successfully: Data.xlsx"
                    )
                )
                .catch((error) =>
                    console.error("Error saving sensor data file:", error)
                );
        })
        .catch((error) => {
            console.error("Error creating sensor data file:", error);
        });
};
